...
namespace gf {
    ...
    //Приведения вычесления таблиц, данное пояснение я указал в help_gf
    ...
    /* ################################
     * # ДЕЯТЕЛЬНОСТЬ НА МЕСТАХ ГАЛУБА #
     * ################################ */

     /* @brief Дополнение в полях Галуа
    ...
    * @return x / y */
    inline int div_my(int x, int y) {
        assert(y != 0);
        if (x == 0) return 0;
        return exp_[(log_[x] + 255 - log_[y]) % 255]; // прибавляем 255, чтобы убедиться, что разница не отрицательная
    }

    /*@ краткое X в степени Y w
        * @param x - операнд
    ...
    /* ##########################
     * # ПОЛИНОМИЧЕСКИЕ ОПЕРАЦИИ #
     * ########################## */


    /*@brief Многочлен умножения на скаляр
        * @param & p - исходный полином
        * @param x - скаляр
        * @return -полином результата */
    ...
    /*@brief Сложение двух многочленов
        * @param & p - полином правого операнда
        * @param & q - полином левого операнда
        * @ полином назначения возврата */
    ...
    /*@brief Умножение двух многочленов
        * @param & p - полином правого операнда
        * @param & q - полином левого операнда
        * @ полином назначения возврата */
    std::vector<int> gf_poly_mult(std::vector<int>& p, std::vector<int>& q) {
        ...
        // умножаем каждый коэффициент p на все коэффициенты q
    ...
    /*@brief Деление двух многочленов
        * @param & divident - полином правого операнда
        * @param & divisor - полином левого операнда
        * @param & quotient - многочлен назначения
        * @param & stayder - многочлен назначения */
    void gf_poly_div(std::vector<int>& divident, std::vector<int>& divisor, std::vector<int>& quotient, std::vector<int>& remainder) {
        ...
        // используем алгоритм синтетического деления
        for (int i = 0; i < divident.size() - (divisor.size() - 1); i++) {
            int coef = msg_out[i];
            if (coef != 0) {                                          // log [0] не определен, поэтому нам нужно вручную проверить этот случай
                for (int j = 1; j < divisor.size(); j++) {
                    if (divisor[j] != 0) {
                        msg_out[i + j] ^= mult(divisor[j], coef);      // ~ msg_out [i + j] + = - divisor [j] * кайфовый, но xor быстрее
                    }
                }
            }
        }
    ...
    // Вычислить многочлен в поле, заданном значением x
    /*@ краткое вычисление многочлена от x
        * @param & p - многочлен для вычисления
        * @param x - точка оценки */
